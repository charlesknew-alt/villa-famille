const Flights = {
  ...
  withTimeout(promise, ms) {
    return new Promise((resolve, reject) => {
      const t = setTimeout(() => reject(new Error("timeout")), ms || 3000);
  ...
  async load() {
    ...
    try {
      const res = await this.withTimeout(fetch("data/fares.json", { cache: "no-store" }), 3000);
      if (res.ok) this.cache = await res.json();
    } catch (_) { /* file://, offline, or hung */ }
  ...
  skyLoaderPromise: null,

  ensureSkyLoader() {
    if (window.skyscanner && window.skyscanner.widgets) return Promise.resolve();
    if (this.skyLoaderPromise) return this.skyLoaderPromise;
    this.skyLoaderPromise = new Promise((resolve, reject) => {
      const done = () => {
        if (window.skyscanner && window.skyscanner.widgets) resolve();
        else reject(new Error("Skyscanner widget failed to load"));
      };
      const existing = document.getElementById("skyscanner-widget-loader");
      if (existing) {
        if (window.skyscanner && window.skyscanner.widgets) return resolve();
        existing.addEventListener("load", done);
        existing.addEventListener("error", () => reject(new Error("Skyscanner widget failed to load")));
        return;
      }
      const s = document.createElement("script");
      s.id = "skyscanner-widget-loader";
      s.src = "https://widgets.skyscanner.net/widget-server/js/loader.js";
      s.async = true;
      s.onload = done;
      s.onerror = () => reject(new Error("Skyscanner widget failed to load"));
      document.body.appendChild(s);
    });
    return this.skyLoaderPromise;
  },

  widgetHtml(opts) {
    const o = opts || {};
    const from = o.from || "LGW";
    const to = o.to || "TLN";
    const date = o.date || "";
    const back = o.back || "";
    return '<div class="sky-widget-wrap" aria-label="Skyscanner live flight search">' +
      '<div data-skyscanner-widget="FlightSearchWidget"' +
      ' data-locale="en-GB" data-market="UK" data-currency="GBP"' +
      ' data-origin-iata-code="' + from + '"' +
      ' data-destination-iata-code="' + to + '"' +
      (date ? ' data-flight-outbound-date="' + date + '"' : "") +
      (back ? ' data-flight-inbound-date="' + back + '"' : "") +
      ' data-flight-type="' + (back ? "return" : "one way") + '"' +
      ' data-colour="#4f5d3a"' +
      ' data-font-colour="#f6f0e6"' +
      ' data-button-colour="#c25b3c"' +
      ' data-button-font-colour="#ffffff"' +
      ' data-powered-by-logo-colour="light"' +
      ' data-widget-border-radius="16"' +
      ' data-widget-padding="16px"' +
      ' data-direct-flights="true"' +
      ' data-button-label="Search live prices"' +
      ' data-target="_blank"' +
      ' data-web-only-redirects="true"></div></div>';
  },

  mountWidget() {
    this.ensureSkyLoader().then(() => {
      if (window.skyscanner && window.skyscanner.widgets) window.skyscanner.widgets.load();
    }).catch(() => {
      document.querySelectorAll(".sky-widget-wrap").forEach((el) => {
        el.innerHTML = '<p class="muted">Skyscanner needs the internet. <a href="' +
          this.skyscannerUrl("LGW", "TLN") + '" target="_blank" rel="noopener">Open skyscanner.net</a></p>';
      });
    });
  },
  ...
  async estimateReturn(arrival, departure, guests) {
    ...
    let fares = [];
    try {
      fares = await this.withTimeout(this.getFares({ date: arrival, back: departure, adults: n }), 3000);
    } catch (_) {
