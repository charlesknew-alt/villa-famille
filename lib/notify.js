window.Notify = {
  ...
  openProblems() {
    return (Store.openIssues ? Store.openIssues() : (Store.data.maintenance || []).filter((m) => m && m.status !== "completed"))
      .slice()
      .sort((a, b) => String(b.createdAt || b.date || "").localeCompare(String(a.createdAt || a.date || "")));
  },

  notifiedProblemIds(user) {
    const raw = (user && user.emailProblemsNotifiedIds) || [];
    return Array.isArray(raw) ? raw.filter(Boolean) : [];
  },

  newProblemsFor(user) {
    const seen = {};
    this.notifiedProblemIds(user).forEach((id) => { seen[id] = true; });
    return this.openProblems().filter((m) => m && m.id && !seen[m.id]);
  },

  rememberProblemsNotified(user, problems) {
    if (!user) return;
    const ids = (problems || []).map((m) => m && m.id).filter(Boolean);
    if (!ids.length) return;
    const seen = {};
    const next = [];
    this.notifiedProblemIds(user).concat(ids).forEach((id) => {
      if (!id || seen[id]) return;
      seen[id] = true;
      next.push(id);
    });
    user.emailProblemsNotifiedIds = next.slice(-200);
    user.updatedAt = new Date().toISOString();
  },

  problemLine(m) {
    if (!m) return "";
    const pri = m.priority ? String(m.priority) : "important";
    const status = m.status || "reported";
    const when = m.date || (m.createdAt ? String(m.createdAt).slice(0, 10) : "");
    return (m.title || "Issue") + " · " + pri + " · " + status + (when ? " · " + when : "");
  },

  digestMessage(freshBookings, newProblems) {
    const parts = ["Hello,\n", "Here is your Family House update.\n"];
    if (newProblems && newProblems.length) {
      parts.push("New / unreported problems:");
      newProblems.slice(0, 20).forEach((m) => parts.push("• " + this.problemLine(m)));
      parts.push("");
    }
    if (freshBookings && freshBookings.length) {
      parts.push("Booking news:");
      freshBookings.slice(0, 12).forEach((ev) => parts.push("• " + ev.summary));
      parts.push("");
    }
    parts.push("Open https://france.directestates.co.uk to see the house.");
    parts.push("You can change email frequency in Settings.");
    return parts.join("\n");
  },
  ...
  async flushDigests() {
    ...
    for (let m = 0; m < modes.length; m++) {
      ...
      for (let i = 0; i < people.length; i++) {
        ...
        const newProblems = this.newProblemsFor(u);
        // Weekly: only send when there are new problems (never the same list again).
        // Every 3 days: send if there are new problems or new booking news.
        if (pref === "weekly") {
          if (!newProblems.length) continue;
        } else if (!newProblems.length && !fresh.length) {
          continue;
        }
        const subject = newProblems.length
          ? ("Family House · " + newProblems.length + " new problem" + (newProblems.length === 1 ? "" : "s"))
          : "Family House booking update";
        const message = this.digestMessage(fresh, newProblems);
        if (await this.sendOne(u.email, u.name || u.firstName || "Family", subject, message)) {
          sent += 1;
          u.emailLastNotifiedAt = new Date().toISOString();
          this.rememberProblemsNotified(u, newProblems);
          Store.rememberUser(u);
        }
      }
    }
