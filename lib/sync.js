...
window.FamilySync = {
  ...
  keys: ["users", "removedIds", "removedBookingIds", "bookings", "reviews", "places", "expenses", "maintenance", "comments", "owners", "settings", "notifyEvents"],
  lastError: "",
  ...
  sanitize(payload) {
    ...
    return {
      ...
      removedBookingIds: Array.isArray(src.removedBookingIds) ? src.removedBookingIds.filter(Boolean) : [],
      ...
      notifyEvents: this.stripPhotos(src.notifyEvents || []),
      settings: {
  ...
  usersForBlob(list) {
    return (list || []).filter((u) => u && u.id).map((u) => {
      const out = {
        ...
        role: u.role || "family",
        email: u.email || "",
        emailNotify: u.emailNotify || "off",
        emailLastNotifiedAt: u.emailLastNotifiedAt || "",
        emailProblemsNotifiedIds: Array.isArray(u.emailProblemsNotifiedIds)
          ? u.emailProblemsNotifiedIds.filter(Boolean).slice(-200)
          : [],
        pinSalt: u.pinSalt || "",
        pinHash: u.pinHash || "",
  ...
  stamp(item) {
    if (!item) return 0;
    const times = [item.updatedAt, item.cancelledAt, item.confirmedAt, item.createdAt]
      .map((v) => Date.parse(v || 0) || 0);
    return Math.max.apply(null, times.concat([0]));
  },
  ...
  stripPhotos(value) {
    ...
    if (Array.isArray(value)) {
      ...
      for (let i = 0; i < value.length; i++) out[i] = this.stripPhotos(value[i]);
      return out;
    }
    const out = {};
    ...
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i];
      if (key === "photos" || key === "receipts" || key === "videos" || key === "invoices") {
        out[key] = [];
        continue;
      }
      if (key === "photo" || key === "image" || key === "dataUrl") continue;
      const v = value[key];
      // Skip huge data-URL blobs without walking them.
      if (typeof v === "string" && v.length > 400 && v.slice(0, 5) === "data:") continue;
      out[key] = (v && typeof v === "object") ? this.stripPhotos(v) : v;
    }
    return out;
  },
  ...
  familySlice(data) {
    const src = data || {};
    const out = {
      removedIds: src.removedIds || [],
      removedBookingIds: src.removedBookingIds || []
    };
    this.keys.forEach((key) => {
      if (key === "removedIds" || key === "removedBookingIds") return;
      if (key === "settings") {
        out.settings = {
          updatedAt: (src.settings && src.settings.updatedAt) || "",
          schoolHolidayNote: (src.settings && src.settings.schoolHolidayNote) || ""
